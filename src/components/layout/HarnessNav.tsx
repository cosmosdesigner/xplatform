...
import { Terminal, GraduationCap, Menu, X } from "lucide-react";
...
export function HarnessNav() {
  ...
  return (
    <>
        ...
        <Container>
          <div className="flex h-14 items-center justify-between gap-8">
            <div className="flex items-center gap-3">
              ...
              <a href="#" className="flex items-center gap-1.5 text-[#ededed] font-semibold text-sm tracking-tight" aria-label="AI Coding Harnesses Course — top">
                <Terminal className="h-4 w-4 text-[#10b981]" aria-hidden="true" />
                <span className="font-mono text-xs text-[#878787] tracking-widest uppercase">Harnesses</span>
              </a>
            </div>
            ...
            <button ref={hamburgerRef} className="md:hidden p-2 text-[#a0a0a0] hover:text-[#ededed] transition-colors" onClick={() => setMobileOpen(!mobileOpen)} aria-expanded={mobileOpen} aria-controls="harness-mobile-menu" aria-label={mobileOpen ? "Close menu" : "Open menu"}>
      ...
      {mobileOpen && (
        <div id="harness-mobile-menu" role="dialog" aria-modal="true" aria-label="Navigation menu" className="fixed inset-0 z-40 bg-[#080808]/98 flex flex-col pt-14" onKeyDown={(e) => {
          if (e.key !== "Tab") return;
