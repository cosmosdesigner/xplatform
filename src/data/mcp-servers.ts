...
const module1Slides: Slide[] = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        text: "Enterprise teams have dozens of internal systems: ADO boards, Wiki pages, internal APIs, databases, design tools, CI pipelines. MCP means you can expose any of them to AI agents through a standard protocol — without building custom integrations per agent tool.",
      },
    ] as ContentBlock[],
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        text: "'MCP server' refers to the program that serves context data, regardless of where it runs. A server can be a local subprocess on your machine (stdio transport) or a remote HTTP service. The @azure-devops/mcp server you use daily runs as a local subprocess spawned by npx.",
      },
    ] as ContentBlock[],
  },
  {
...
const module3Slides: Slide[] = [
  {
    number: 1,
    title: "Enterprise MCP: Azure DevOps case study",
    headline: "A real production\nMCP server\nyou use daily.",
    sections: [
      {
        type: "paragraph",
        text: "The @azure-devops/mcp server is a real production MCP server used by enterprise teams daily. It exposes tools for managing work items, Wiki pages, PRs, branches, and builds. Understanding its design patterns teaches you how to build enterprise-grade MCP servers.",
      },
      {
        type: "comparison",
        headers: ["Capability", "What it exposes"],
        rows: [
          ["Work Items", "Read, create, patch fields, update tags, append comments, link parent-child, link branches"],
          ["Wiki", "Read pages, create pages, append sections, update sections"],
          ["Pull Requests", "Read, create, update, link work items"],
          ["Branches", "Read metadata, validate naming conventions"],
          ["Builds/Deployments", "Read-only evidence — build results, test results, deployment status"],
        ],
      },
      {
        type: "callout",
        ...
        text: "The ADO MCP server prefixes all tools with 'azure-devops_' (e.g., azure-devops_get_work_item). This namespacing prevents collisions when multiple MCP servers are connected simultaneously. Follow this pattern: prefix your tools with your server name.",
      },
    ] as ContentBlock[],
  },
  {
    ...
    sections: [
      {
        ...
        rows: [
          ["Azure CLI (azcli)", "Enterprise ADO — uses your existing 'az login' session. Most secure — no tokens in config files."],
          ["Personal Access Token (PAT)", "GitHub, ADO when CLI unavailable. Store in environment variable, never in config files."],
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        text: "Always use narrow, field-level updates. If you need to update the tags on a work item, read the current tags, compute the diff, and patch only the tags field. Never send a full work item replacement — you'll overwrite changes made by other users between your read and write.",
      },
    ] as ContentBlock[],
  },
  {
...
const module5Slides: Slide[] = [
  ...
  },
  {
    ...
    sections: [
      {
        type: "paragraph",
        text: "Enterprise governance rules describe operations as portable intents — 'wiki.create_page', 'work_item.update_tags' — not as specific tool names. This decouples governance from the MCP server implementation, so you can switch servers without rewriting rules.",
      },
      {
        type: "code",
        caption: "Operation intent envelope",
        code: "operation:\n  intended_operation: work_item.update_tags\n  target:\n    organization: ptbcp\n    project: IT.DIT\n    artifact_type: Work Item\n    artifact_id: FEAT-102\n  source_of_truth: Work Items\n  required_reads:\n    - current tags\n    - current revision\n  conflict_checks:\n    - stale revision\n    - invalid tag (not in canonical set)\n  failure_status: failed",
      },
      {
        type: "callout",
        variant: "insight",
        title: "Why intent matters",
        text: "If your governance rule says 'call azure-devops_update_work_item', you're locked to that server's API. If it says 'work_item.update_tags', any MCP server that can update work item tags satisfies the intent. This is the difference between coupling to implementation and coupling to capability.",
      },
    ] as ContentBlock[],
  },
  {
    ...
    sections: [
      {
        type: "paragraph",
        text: "Enterprise systems store the same information in multiple places — Wiki pages, work items, PRs, and the repository itself. Without an explicit ownership map, agents will read from the wrong source or write to the wrong target.",
      },
      {
        type: "comparison",
        ...
        rows: [
          ...
          ["State, tags, assignment, relationships", "Work Items"],
          ["Code review findings, CI evidence", "Pull Requests"],
          ["Build results, deployment status", "CI/CD systems (read-only)"],
      ...
      },
      {
        ...
        text: "When Wiki and Work Item disagree on the same fact, the authoritative source for that field type wins. If ownership is unclear, return status: conflict with both sources identified — let the human decide. Agents MUST NOT guess which source is correct.",
      },
    ] as ContentBlock[],
  },
  {
...
export const mcpFrameworks = [
  ...
  },
  {
    ...
    items: [
      ...
      "Namespace: prefix with server name (azure-devops_, github_, jira_)",
    ],
  },
];
